...
void recordTask(void * pwParameters){
    ...
    while (1){
        ...
        //This will be terribly ineffecient, deffo need to consider a better way to do this. 
        if (playback){
            ...
            for (int i = 0; i < playbackBuffer.size(); i++){
                if (playbackBuffer[i][4] == counterUpper && playbackBuffer[i][5] == counterLower){
                    //Can hopefully just add it to the CAN bus (WON'T WORK FOR ITS OWN VALUES)
                    xQueueSend( msgOutQ, playbackBuffer[i], portMAX_DELAY);
                    //Below writes directly to the notesPlayed
                    // if (playbackBuffer[i][0] == 'R'){
                    //     int index = playbackBuffer[i][2];
                    ...
                    //     {
                    //         if(notesBuffer.notesPlayed[index][i] == playbackBuffer[i][1])
                    //         {
                    ...
                    //     }
                    //     }
                    //     else if (playbackBuffer[i][0] == 'P')
                    //     {
                    //     notesBuffer.notesPlayed[(int)index].push_back((uint8_t)playbackBuffer[i][1]);
                    //     }
                }
            }
